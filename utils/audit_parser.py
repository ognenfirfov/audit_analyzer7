...
def ask_ai_to_summarize(text):
    """Uses GPT-4 to summarize key audit elements from the text."""
...
def process_audits(file_paths):
    """Processes a list of PDF paths and returns a comparison table."""
    data = []
    for i, path in enumerate(file_paths):
        text = extract_text_from_pdf(path)
        summary = ask_ai_to_summarize(text)

        # Parse structured summary into dictionary
